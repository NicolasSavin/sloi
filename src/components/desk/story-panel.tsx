...
import type { TvBrief } from "@/lib/tv-brief";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        {confidence != null ? <span className="font-mono text-xs tabular-nums text-muted">{confidence}%</span> : <span className="text-xs text-dim">движок</span>}
      </div>
      <div>
      ...
      {setup ? (
        <div className="rounded-lg panel-volume p-3">
          ...
          <dl className="mt-2 space-y-1 text-sm text-muted">
            <div><span className="text-dim">вход </span>{setup.entry}</div>
            <div><span className="text-dim">стоп </span>{setup.stop}</div>
            <div><span className="text-dim">цели </span>{setup.targets.join(" · ") || "—"}</div>
          </dl>
        </div>
      ) : null}
      ...
      {risks?.length ? (
        <div>
          ...
          <ul className="mt-1 list-disc space-y-1 pl-4 text-sm text-muted">{risks.map((r) => <li key={r}>{r}</li>)}</ul>
        </div>
      ) : null}
      {watch?.length ? (
        <div>
          <p className="text-xs font-medium tracking-wide text-dim">Что смотреть дальше</p>
          <ul className="mt-1 list-disc space-y-1 pl-4 text-sm text-muted">{watch.map((r) => <li key={r}>{r}</li>)}</ul>
        </div>
      ) : null}
    </div>
  );
...
export function EtherCard({ ether }: { ether?: TvBrief | null }) {
  if (!ether) return null;
  return (
    <div className="rounded-lg panel-volume p-3">
      <p className="text-[10px] font-mono tracking-[0.16em] text-accent">ЭФИР · СПРАВКА</p>
      <p className="mt-2 text-sm font-medium">{ether.onAir}</p>
      <ul className="mt-2 space-y-1 text-sm text-muted">{ether.titles.slice(0, 3).map((x) => <li key={x}>{x}</li>)}</ul>
      <p className="mt-2 text-xs text-dim">{ether.note}</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg panel-volume p-3">
      ...
      <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 font-mono text-xs text-muted">
        <div><span className="text-dim">страйк </span>{c.strike}</div>
        <div><span className="text-dim">эксп. </span>{c.expiry}</div>
        <div className="col-span-2"><span className="text-dim">доска </span>{c.source}</div>
      </dl>
