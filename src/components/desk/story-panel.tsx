  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {headline ? (
          <h2 className="mt-1 text-base font-medium leading-snug">{headline}</h2>
        ) : null}
...
export function StoryBody({
  ...
  return (
    <div className="space-y-4">
      ...
      {aiError ? (
        <div className="rounded-md bg-elevated p-3 text-sm text-warn">{aiError}</div>
      ) : null}
      {brief ? (
        <StoryPanel
        ...
        <StoryPanel
          ...
          setup={{
            type: "",
            entry:
              snap.localSetup.entry != null
                ? formatPrice(snap.localSetup.entry, decimals)
                : "ждать реакцию у края",
            stop:
              snap.localSetup.stop != null
                ? formatPrice(snap.localSetup.stop, decimals)
                : snap.localSetup.invalidation,
            targets: snap.localSetup.targets.map((n) => formatPrice(n, decimals)),
            invalidation: snap.localSetup.invalidation,
          }}
        />
      ) : null}
      <p className="text-xs text-dim">Разбор — цепочка причина → следствие, не сигнал.</p>
    </div>
  );
}
