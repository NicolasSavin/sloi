...
function drawCreature(
  ...
  const t = now / 1000;
  const up = kind === "bull";
  const hop = Math.abs(Math.sin(t * 5.4)) * 16;
  const sway = Math.sin(t * 2.2) * 0.1;
  ctx.save();
  ctx.lineJoin = "round";
  ctx.lineCap = "round";
  ctx.translate(Math.sin(t * 2.6) * 5, -hop);
  ctx.rotate(sway);
  ctx.fillStyle = "rgba(0,0,0,0.38)";
  ctx.beginPath();
  ctx.ellipse(2, 54 + hop * 0.35, 30 - hop * 0.45, 8, 0, 0, Math.PI * 2);
  ctx.fill();

  const lite = up ? "#ffe9a8" : "#f0c8a0";
  const mid = up ? "#e0a030" : "#a06038";
  const deep = up ? "#4a2808" : "#2a140c";
  const body = ctx.createRadialGradient(-12, 4, 6, 2, 16, 30);
  body.addColorStop(0, lite);
  body.addColorStop(0.4, mid);
  body.addColorStop(1, deep);
  ctx.fillStyle = body;
  ctx.beginPath();
  ctx.ellipse(0, 16, 28, 24, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  const head = ctx.createRadialGradient(-6, -20, 4, 0, -12, 22);
  head.addColorStop(0, lite);
  head.addColorStop(0.5, mid);
  ...
  ctx.beginPath();
  ctx.ellipse(-6, -22, 6, 4, -0.4, 0, Math.PI * 2);
  ctx.fill();
  ...
  if (up) {
    ...
    ctx.beginPath();
    ctx.moveTo(-8, -28);
    ctx.quadraticCurveTo(-18, -48, -4, -30);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(10, -28);
    ctx.quadraticCurveTo(22, -48, 8, -30);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  } else {
    ctx.fillStyle = mid;
    ctx.beginPath();
    ctx.ellipse(-14, -28, 7, 10, -0.4, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(14, -28, 7, 10, 0.4, 0, Math.PI * 2);
    ctx.fill();
  ...
  const blink = Math.sin(t * 1.15) > 0.94 ? 0.15 : 1;
  const look = up ? -1.5 : 2;
  for (const sx of [-6, 6]) {
    ctx.fillStyle = "#fff";
    ctx.beginPath();
    ctx.ellipse(sx, -16, 5.2, 5.2 * blink, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (blink > 0.4) {
      ...
      ctx.beginPath();
      ctx.arc(sx + look * 0.3, -16 + look * 0.2, 2.2, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "#fff";
      ctx.beginPath();
      ctx.arc(sx - 1.4, -17.4, 1, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.strokeStyle = "#1a1008";
  ctx.lineWidth = 2;
  ctx.beginPath();
  if (up) ctx.arc(0, -10, 7, 0.2, Math.PI - 0.2);
  else ctx.arc(0, -6, 7, Math.PI + 0.2, -0.2);
  ctx.stroke();

  const arm = Math.sin(t * 6) * 0.45 + (up ? -1.05 : 1.1);
  ctx.save();
  ctx.translate(22, 4);
  ctx.rotate(arm);
  const armG = ctx.createLinearGradient(0, 0, 0, 22);
  armG.addColorStop(0, lite);
  armG.addColorStop(1, deep);
  ...
  ctx.beginPath();
  ctx.moveTo(0, 24);
  ctx.lineTo(0, up ? -10 : 42);
  ctx.lineTo(-6, up ? 0 : 34);
  ctx.moveTo(0, up ? -10 : 42);
  ctx.lineTo(6, up ? 0 : 34);
  ctx.stroke();
  ctx.restore();

  for (let i = 0; i < 6; i++) {
    const ph = t * (2.2 + i * 0.15) + i;
    const fly = (ph % 2.8) / 2.8;
    drawBill(
      ctx,
      -30 - i * 3 + Math.sin(ph * 2) * 12,
      (up ? -6 : 12) + (up ? -1 : 1) * fly * 38 + Math.cos(ph * 2.4) * 6,
      ph * 1.4,
      ph * 3,
    );
  }
