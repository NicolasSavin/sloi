...
export async function askVision(
  ...
  const keys = keyStatus();
  if (!keys.any) {
    return { miss: "Нет ключа для разбора картинки. GROQ_API_KEY или GEMINI_API_KEY в Vercel." };
  }
  const vision = [
    {
      label: "Gemini",
  ...
  for (const p of vision.filter((x) => x.key)) {
    for (const model of p.models) {
      try {
        const res = await fetch(p.url, {
          method: "POST",
        ...
        const text = json.choices?.[0]?.message?.content?.trim();
        if (text && text.length > 40) return { text, model: p.label };
        errors.push(`${p.label} пустой`);
      } catch {
        errors.push(`${p.label} сеть`);
      }
    }
  }
  return { miss: errors.slice(0, 4).join(" · ") || "картинку не разобрали" };
}
