...
const SYSTEM =
  "Ты рассказываешь, что делает крупный игрок (smart money). Одна история, по-русски. Если во входе есть сценарии ФРС/NFP/CPI — используй их вероятности, не выдумывай свои проценты. Скажи куда цена чаще идёт и за какое время. Не обещай прибыль. Не выдумывай уровни. Отвечай ТОЛЬКО JSON.";

function userPrompt(payload: unknown) {
  const data = JSON.stringify(payload);
  const cut = data.length > 7000 ? `${data.slice(0, 7000)}…` : data;
  return `Одна история: что делает крупняк, чего ждёт, к чему это приведёт. Учти сентимент и блок макро-сценариев (ФРС, Пауэлл, NFP): куда чаще и как скоро, это шаблон истории, не предсказание.
JSON:
{
  "bias": "bullish" | "bearish" | "range",
  "confidence": 0-100,
